...
// Transient network blips (undici "fetch failed", DNS hiccups, socket drops) —
// worth 2 quick retries; they are NOT rate-limit signals.
const NETWORK_ERR_RE =
  /fetch failed|fetch timed out|ECONNRESET|ECONNREFUSED|ETIMEDOUT|ENOTFOUND|EAI_AGAIN|socket hang up|terminated|network/i;
...
export async function complete(messages) {
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      ...
      const busy = BUSY_ERR_RE.test(msg);
      if (!busy && NETWORK_ERR_RE.test(msg) && attempt < 2) {
        console.error(`[llm] network blip (attempt ${attempt + 1}) — retrying in 3s`);
...
export async function webSearch(query, num = 8, recencyDays = null) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      lastErr = err;
      if (attempt === 1 || !NETWORK_ERR_RE.test(String(err?.message || err || ''))) throw err;
