...
/** Compact recent-conversation block so "our agency"/"that site" resolve from context */
...
export async function runAgent(text, user) {
  ...
  if (pendingLeadSend.has(user.id)) {
    ...
    if (Date.now() - p.at <= LEAD_TTL_MS && !looksLikeFreshCommand(t)) {
      ...
      if (/^(send|send all|send them|send it|yes|yep|yeah|go|go ahead|approve|approved|dispatch|do it|ok|okay)\b/i.test(t)) {
        const results = [];
        for (const d of p.drafts.slice(0, MAX_LEADS)) {
          if (!d?.email || !d?.body) continue;
          try {
            const r = await zapierCallTool('gmail_send_email', {
              to: d.email,
              subject: d.subject || 'Quick question',
              body: d.body,
              output_hint: 'confirmation with the message id',
            });
        ...
        return ok('📤 Outreach dispatched', `**${sent}/${p.drafts.length}** emails sent from your Gmail:\n\n${results.join('\n')}\n\n📍 Watch your inbox for replies — say "find clients" anytime for the next batch.`, sent ? OK_COLOR : 0xef4444);
      }
